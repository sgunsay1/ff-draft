...
const generateAdp = () =>
  new Promise((resolve, reject) => {
    ...
    const adps: Array<Promise<Player> | undefined> = [];
    fs.createReadStream("./src/seeders/adp.csv")
      ...
      .on("data", async (row) => {
        try {
          ...
          if (player) {
            adps.push(player?.update({ teamName: row[2], adp }));
          } else {
      ...
      .on("error", (error) => reject(error.message))
      .on("end", () => resolve(Promise.all(adps)));
  });
    ...
    fs.createReadStream("./src/seeders/prices.csv")
      ...
      .on("data", async (row) => {
        try {
          ...
          const suggestedPrice = parseInt(row[1]);
          if (player) {
            await player?.update({ suggestedPrice });
          }
        } catch (e) {
